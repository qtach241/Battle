--[[
Timers Library by BMD

Example Usage:

1.  Create a timer running every second that starts immediately on the next frame,
    respects pauses:
...
2.  Create a timer, same as above, with a shorthand call:
...
3.  Create a timer which calls a function with a table context:

    Timers:CreateTimer(GameMode.someFunction, GameMode)

4.  Create a timer running every second that starts 5 seconds in the future, 
    respects pauses:
...
5.  Create a timer, 10 second delayed, run once using gametime, respect pauses.
    (endTime is when the timer should first execute, can omit if you want it to
    run first on the next frame):
...
7.  Create a timer, 10 second delayed, run once regardless of pauses:
